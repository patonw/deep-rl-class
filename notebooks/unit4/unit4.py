...
def reinforce(policy, optimizer, n_training_episodes, max_t, gamma, print_every):
    ...
    for i_episode in tqdm(range(1, n_training_episodes+1)):
        ...
        for i in range(1, n_steps):
            t = n_steps - i
            returns = scores[t+1] + gamma * returns[t+1]
...
# %% id="NCNvyElRStWG"
def reinforce(policy, optimizer, n_training_episodes, max_t, gamma, print_every, *, reset_info=False):
    # Help us to calculate the score during the training
    scores_deque = deque(maxlen=100)
    scores = []
    # Line 3 of pseudocode
...
# %% id="Lo4JH45if81z"
max_frames = cartpole_hyperparameters['max_t']
...
def record_video(env, policy, out_directory, fps=30, max_frames=1000):
    ...
    state, *_ = env.reset()
    img = env.render() #env.render(mode='rgb_array')
    images.append(img)
